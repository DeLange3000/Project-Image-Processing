...
for i = 1:superpixel_height:h_input %assign each input pixel to a superpixel
    for j = 1:superpixel_width:w_input
        ...
        input_superpixels(ir:ir+round(superpixel_height) + 1, jr:jr+round(superpixel_width) + 1) = counter;
        counter = counter + 1;
    end
end
...
while temperature > temperature_f
    ...
    fprintf('iteration ' + string(loop_index) + '\tT = ' + string(temperature) + '\t\tK = ' + string(size(palette,2)) + '\n')
...
%    m = 45;
    ...
    %this)
			...
			% do color stuff once for entire palette instead of for each
			% superpixel
			%color
                pc_input = input_imag;
				dc = zeros(h_input, w_input, size(palette,2));
			for col = 1:size(palette, 2)
                pc_super = palette(:, col);
                
				dc_a1 = (pc_input(:,:,1) - pc_super(1));
				dc_a2 = (pc_input(:,:,2) - pc_super(2));
				dc_a3 = (pc_input(:,:,3) - pc_super(3));

				dc(:,:,col) = sqrt(dc_a1.^2 + dc_a2.^2 + dc_a3.^2);
			end
            ...
            for aa = 1:N
			    ...
			    d_min = dc(superpixel_palette_colors(aa))+dp;
